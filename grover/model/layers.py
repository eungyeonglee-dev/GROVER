...
class MPNEncoder(nn.Module):
    ...
    def forward(self,
        ...
        for _ in range(ndepth - 1):
            ...
            message = message.cuda(device_id)
            self.W_h = self.W_h.cuda(device_id)
            ...
            if self.dense:
                self.act_func = self.act_func.cuda(device_id)
                message = self.act_func(message)  # num_bonds x hidden_size
            else:
                self.act_func = self.act_func.cuda(device_id)
                message = self.act_func(input + message)
            self.dropout_layer = self.dropout_layer.cuda(device_id)
...
class Head(nn.Module):
    ...
    def __init__(self, args, hidden_size, atom_messages=False):
        ...
        mpn_v = MPNEncoder(args=args,
                                ...
                                input_layer="none",
                                dynamic_depth="truncnorm")
        self.mpn_q = mpn_q.cuda(0)
        self.mpn_k = mpn_k.cuda(1)
        self.mpn_v = mpn_v.cuda(2)
    ...
    def forward(self, f_atoms, f_bonds, a2b, a2a, b2a, b2revb):
        ...
        #init_messages = init_messages.cuda(0)
        #init_attached_features = init_attached_features.cuda(0)
        #a2nei = a2nei.cuda(0)
        #a2attached = a2attached.cuda(0)
        #b2a = b2a.cuda(0)
        #b2revg = b2revb.cuda(0)

        q = self.mpn_q(init_messages=init_messages.cuda(0),
                       init_attached_features=init_attached_features.cuda(0),
                       a2nei=a2nei.cuda(0),
                       a2attached=a2attached.cuda(0),
                       b2a=b2a.cuda(0),
                       b2revb=b2revb.cuda(0),
                       device_id = 0)

        #init_messages = init_messages.cuda(1)
        #init_attached_features = init_attached_features.cuda(1)
        #a2nei = a2nei.cuda(1)
        #a2attached = a2attached.cuda(1)
        #b2a = b2a.cuda(1)
        #b2revg = b2revb.cuda(1)

        k = self.mpn_k(init_messages=init_messages.cuda(1),
                       init_attached_features=init_attached_features.cuda(1),
                       a2nei=a2nei.cuda(1),
                       a2attached=a2attached.cuda(1),
                       b2a=b2a.cuda(1),
                       b2revb=b2revb.cuda(1),
                       device_id = 1)

        #init_messages = init_messages.cuda(2)
        #init_attached_features = init_attached_features.cuda(2)
        #a2nei = a2nei.cuda(2)
        #a2attached = a2attached.cuda(2)
        #b2a = b2a.cuda(2)
        #b2revg = b2revb.cuda(2)

        v = self.mpn_v(init_messages=init_messages.cuda(2),
                       init_attached_features=init_attached_features.cuda(2),
                       a2nei=a2nei.cuda(2),
                       a2attached=a2attached.cuda(2),
                       b2a=b2a.cuda(2),
                       b2revb=b2revb.cuda(2),
                       device_id = 2)
        ...
        #k = k.cuda(0)
        #v = v.cuda(0)
        return q.cuda(0), k.cuda(0), v.cuda(0)
